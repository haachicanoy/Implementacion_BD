...
public class RemoveRepeated implements IOperator{
	...
	String type = "RemoveRepeated";
	Set<List<Object>> items ;// Variable que almacena los registros unicos 

	
	@Override
	public String setOperatorName(String name) {
	...
	public String apply(Node node) {		
		if(node.getType().equals(this.type)){
			int limit = 10;
			int blockCount = 1;
			...
			while (scan.hasNext()) {
				...
				if(items.add(rowObject)){							//Identifico los repetido si se añade correctamente true (no hay duplicado), false (hay duplicado)
					try {
						...
						System.out.println("Pt:"+ rowStr);
						blockOutput.write(rowStr);					//Se escribe en el fichero
						blockOutput.newLine();
						} catch (IOException e) {
							e.printStackTrace();
						}							
	...
	public BufferedWriter createOutputFile(String dirStr, String tableName, int block){
        ...
        try {
    		File file = new File(dir+"/"+tableName+"_"+block+".csv");        	        	
